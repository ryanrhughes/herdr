...
Set `ui.show_hostname = true` to show the Herdr server's hostname at the right edge of the desktop tab bar. Remote clients therefore show the remote server name rather than the client device name. Long hostnames are truncated to preserve room for tabs.
